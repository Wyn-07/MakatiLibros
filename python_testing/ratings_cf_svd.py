...
def get_collaborative_filtering_recommendations(patrons_id, top_n):
    ...
    for book_id in recommended_books:
        title_match = df.loc[df['Book ID'] == book_id, 'Title']
        if not title_match.empty:
            recommended_titles.append(title_match.values[0])
        else:
            recommended_titles.append("Unknown Title")  # Handle missing titles gracefully
