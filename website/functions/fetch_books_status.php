...
if ($status && $patrons_id) {
    ...
    if (count($result) > 0) {
        foreach ($result as $row) {
            ...
            if ($row['borrow_status'] === 'Accepted') {

                $return_title = "";

                $overdue_display = "display: none";


                $borrow_title = "Accepted Date";

                $formattedDate = date("F j, Y", strtotime($row['accepted_date']));
                $formattedTime = date("h:i A", strtotime($row['accepted_time']));
                $dayOfWeek = date("l", strtotime($row['accepted_date']));


                $formattedDateReturn = "You can now claim the book at the library";
                $formattedTimeReturn = "";
                $dayOfWeekReturn = "";
                
            } else if ($row['borrow_status'] === 'Borrowed') {

                $return_title = "Expected Return Date";

                // Calculate the expected return date as 5 days after borrow_date
                $expectedReturnTimestamp = strtotime($row['borrow_date'] . ' +5 days');
                $formattedDateReturn = date("F j, Y", $expectedReturnTimestamp);
                $formattedTimeReturn = date("h:i A", $expectedReturnTimestamp); // You can set a fixed time if needed
                $dayOfWeekReturn = date("l", $expectedReturnTimestamp);

                $overdue_display = "display: flex";

            } else  if ($row['borrow_status'] === 'Returned') {

                $return_title = "Return Date";

                // Use the actual return date if br_status is not "Accepted"
                ...
                $return_title = "Pending";

                // Use the actual return date if br_status is not "Accepted"
                $formattedDateReturn = "Waiting for approval....";
                $formattedTimeReturn = "";
                $dayOfWeekReturn = "";
...
?>
            ...
            <div class="profile-container-white-filter-content">
                ...
                <div class="row row-between">
                    <div>
                        <div class="date-font"><?= $return_title ?></div>
                    ...
                    <div>
                        <div class="books-overdue" style="<?= $overdue_display ?>">Overdue</div>
                    </div>
                </div>
                ...
                if ($row['book_status'] === 'Available' && $row['category_name'] !== 'Circulation') {
                    $statusCategoryText = "Non-circulating";
                    $statusCategoryClass = "unavailable";
                    $hideStatus = false;
                } else {
                    $statusCategoryText = htmlspecialchars($row['book_status']);
                    $statusCategoryClass = ($row['book_status'] === 'Available') ? 'available' : 'unavailable';
                    $hideStatus = true;
                }
                ?>
                ...
                <div class="container-books-contents-full">
                    ...
                    <div class="books-contents-image-profile">
                        <div class="books-status-show <?php echo $statusCategoryClass; ?>" <?php echo $hideStatus ? 'style="display: none;"' : ''; ?>>
                            <?php echo htmlspecialchars($row['book_status']); ?>
                        </div>
                    ...
                    <div class="books-contents">
                        ...
                        <div class="row">
                            ...
                            <div class="tooltipss" id="tooltip-add-ratings">
                                <div class="button button-ratings" onclick="openRateModal(<?= htmlspecialchars($row['book_id']) ?>, <?= htmlspecialchars($row['patrons_id']) ?>, '<?= !empty($row['user_rating']) ? htmlspecialchars($row['user_rating']) : '' ?>')"><img src="../images/star-white.png" alt=""></div>
                ...
                <div class="row row-between">
                    ...
                    <div>
                        <div class="date-font"><?= $borrow_title ?></div>
                        <div class="profile-row">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        books.forEach(book => {
            ...
            // Borrow Button Logic
            if (bookStatus.toLowerCase() === 'unavailable' &&
                bookCategory.toLowerCase() === 'circulation' &&
                bookBorrowStatus.trim() === '') {
            ...
            } else if (bookCategory.toLowerCase() === 'circulation' &&
                bookBorrowStatus.trim().toLowerCase() === 'pending') {
                borrowButton.disabled = true;
                tooltipBorrow.textContent = 'You have already requested to borrow this book.';
                tooltipBorrow.style.display = 'flex';
            } else if (bookCategory.toLowerCase() === 'circulation' &&
                bookBorrowStatus.trim().toLowerCase() === 'accepted') {
                borrowButton.disabled = true;
                tooltipBorrow.textContent = 'You have already requested this book. You can now claim it at the library.';
                tooltipBorrow.style.display = 'flex';
            } else if (bookCategory.toLowerCase() === 'circulation' &&
                bookBorrowStatus.trim().toLowerCase() === 'borrowed') {
                borrowButton.disabled = true;
                tooltipBorrow.textContent = 'You are still borrowing the book. Please return it on time.';
                tooltipBorrow.style.display = 'flex';
            } else if (bookStatus.toLowerCase() === 'available' &&
                bookCategory.toLowerCase() !== 'circulation' &&
                bookBorrowStatus.trim() === '') {
                borrowButton.disabled = true;
