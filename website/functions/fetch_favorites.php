    ...
    $sql = "SELECT *
        FROM (
            SELECT 
                ...
                -- Unified book status logic for favorites
                CASE 
                    WHEN br2.borrow_id IS NOT NULL AND br2.status != 'Returned' THEN 'Unavailable'  -- Borrowed but not returned
                    ELSE 'Available'  -- Not borrowed or has been returned
                END AS book_status
            FROM 
                favorites f
            JOIN 
            ...
            LEFT JOIN 
                borrow br ON b.book_id = br.book_id AND br.patrons_id = ?
            LEFT JOIN 
                ratings r ON b.book_id = r.book_id
            LEFT JOIN 
                borrow br2 ON b.book_id = br2.book_id AND br2.status != 'Returned'  -- Check for borrowed status not returned
            WHERE 
                f.patrons_id = ? AND f.status = 'Added'
            GROUP BY 
                f.favorite_id, 
                b.book_id, 
                b.title, 
