...
def get_latest_rated_book_info(patrons_id):
    ...
    result = cursor.fetchone()
     ...
     # Print the latest rated book information
    # if result:
    #     print(f"Latest rated book - ID: {result[0]}, Title: {result[1]}")
    # else:
    #     print("No rated book found for the given patron ID.")
...
def get_content_based_recommendations(book_id, n_recommendations=number):
    ...
    distances, indices = nn.kneighbors(content_matrix[index], n_neighbors=n_recommendations + number)  # Fetch extra neighbors to handle duplicates
