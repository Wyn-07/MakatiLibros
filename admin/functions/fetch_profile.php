...
try {
    ...
    if ($librarian) {
        ...
        $image = $librarian['image'] ?? 'default-image.jfif';

        // Fetch the librarian ID explicitly and set it in the session
        if (!$isAdmin) {
            // If the user is a librarian, fetch the librarian_id
            $query = "SELECT librarians_id FROM librarians WHERE email = :email";
            $stmt = $pdo->prepare($query);
            $stmt->bindParam(':email', $email);
            $stmt->execute();
            $librarianData = $stmt->fetch(PDO::FETCH_ASSOC);
            $_SESSION['librarian_id'] = $librarianData['librarians_id'] ?? null;
        } else {
             ...
             $stmt->execute();
             $librarianData = $stmt->fetch(PDO::FETCH_ASSOC);
             $_SESSION['admin_id'] = $librarianData['admin_id'] ?? null;
        }
