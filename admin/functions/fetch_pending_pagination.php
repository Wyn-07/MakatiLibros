...
$sortPendingColumn = isset($_GET['sortPendingColumn']) ? $_GET['sortPendingColumn'] : 'borrow_id';
$sortPendingOrder = isset($_GET['sortPendingOrder']) && $_GET['sortPendingOrder'] == 'desc' ? 'DESC' : 'ASC';

// Ensure valid column name
$validColumns = ['borrow_id', 'borrow_datetime', 'return_datetime', 'title', 'patrons_name', 'status'];
if (!in_array($sortPendingColumn, $validColumns)) {
    $sortPendingColumn = 'borrow_id'; // Default
}
...
switch ($sortPendingColumn) {
    ...
    case 'return_datetime':
        ...
        break;
    case 'status':
        $sql .= 'b.status ' . $sortPendingOrder;
        break;
    case 'title':
        // Apply case-insensitive sorting to the title
        $sql .= 'LOWER(TRIM(bk.title)) ' . $sortPendingOrder;
        break;
    case 'patrons_name':
        $sql .= 'CONCAT(u.firstname, " ", u.middlename, " ", u.lastname) ' . $sortPendingOrder;
        break;
    default:
        $sql .= 'b.borrow_id ' . $sortPendingOrder;  
}
...
$totalQuery = "SELECT COUNT(*) 
               ...
               WHERE 
                   ...
                   CONCAT(u.firstname, ' ', u.middlename, ' ', u.lastname) LIKE :searchParam OR
                   b.status LIKE :searchParam)
                   AND b.status = 'Pending'";  // Correct parenthesis grouping for total count
...
echo json_encode([
    ...
    'totalPendingTransaction' => $totalTransaction
]);
?>
