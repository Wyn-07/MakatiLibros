...
function sortTable(columnIndex) {
    ...
    const columnNames = ['date_time', 'page', 'description', 'user_name'];
...
function loadTransactions(page = 1, itemsPerPage = 5, searchQuery = "", sortColumn = 'date_time', sortOrder = '') {
    ...
    fetch(`functions/fetch_audit_pagination.php?page=${page}&itemsPerPage=${itemsPerPage}&search=${encodeURIComponent(searchQuery)}&sortColumn=${sortColumn}&sortOrder=${sortOrder}`)
        .then(response => response.json())
        .then(data => {
            totalLogs = data.totalLogs; // Update total transactions count
            document.getElementById("audit-table-container").innerHTML = generateTable(data.logsList);
                ...
                <td>
                    <div class="td-center">
                        <div class="button-view" 
                            data-log-id="${encodeURIComponent(log.logs_id)}"  
                            data-date-time="${encodeURIComponent(log.date_time)}"
                            data-old-data="${encodeURIComponent(log.old_data)}"
                            data-new-data="${encodeURIComponent(log.new_data)}"
                            data-librarian="${encodeURIComponent(log.user_name)}"
                            data-page="${encodeURIComponent(log.page)}"
                            data-description="${encodeURIComponent(log.description)}"
