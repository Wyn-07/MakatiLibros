...
function sortTable(columnIndex) {
    ...
    const columnNames = ['patrons_name', 'guarantor', 'application_status', 'application_status_reason'];
...
function loadPatrons(page = 1, itemsPerPage = 5, searchQuery = "", sortPatronColumn = 'patrons_name', sortPatronOrder = '') {
    ...
    fetch(`functions/fetch_rejected_application_pagination.php?page=${page}&itemsPerPage=${itemsPerPage}&search=${encodeURIComponent(searchQuery)}&sortPatronColumn=${sortPatronColumn}&sortPatronOrder=${sortPatronOrder}`)
        .then(response => response.json())
        .then(data => {
            totalPatrons = data.totalPatrons; // Update total patrons count
            document.getElementById("rejected-table-container").innerHTML = generateTable(data.patronList);
    ...
    return patronList.map(patron => {
        return `
            <tr>
                ...
                <td>${patron.application_status_reason}</td> 
                <td>
                    <div class="td-center">
