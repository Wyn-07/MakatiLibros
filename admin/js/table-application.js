...
let totalPatrons = 0;
let sortPatronColumn = 'patrons_name';
let sortPatronOrder = 'desc';
let searchQuery = "";

function sortTable(columnIndex) {
    // Map column index to column name for sorting
    const columnNames = ['patrons_name', 'guarantor', 'application_status'];
...
function loadPatrons(page = 1, itemsPerPage = 5, searchQuery = "", sortPatronColumn = 'patrons_name', sortPatronOrder = '') {
    ...
    fetch(`functions/fetch_application_pagination.php?page=${page}&itemsPerPage=${itemsPerPage}&search=${encodeURIComponent(searchQuery)}&sortPatronColumn=${sortPatronColumn}&sortPatronOrder=${sortPatronOrder}`)
        .then(response => response.json())
        .then(data => {
            totalPatrons = data.totalPatrons; // Update total patrons count
            document.getElementById("application-table-container").innerHTML = generateTable(data.patronList);
        ...
        <table id="table">
            <thead>
                <tr>
                    ...
                    `).join('')}
                    <th>
                        <div class="column-title">TOOL</div>
                    </th>
                </tr>
            </thead>
            <tbody>
    ...
    return patronList.map(patron => {
        return `
            <tr>
                ...
                <td>${patron.application_status}</td> 
                <td>
                    <div class="td-center">
                        ...
                        <div class="button-edit" 
                                data-edit-patrons-id="${encodeURIComponent(patron.patrons_id)}"
                                data-edit-guarantor-id="${encodeURIComponent(patron.guarantor_id)}"
                                data-edit-status="${encodeURIComponent(patron.application_status)}"
