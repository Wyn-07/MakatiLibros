...
function loadBooks(page = 1, itemsPerPage = 5, searchQuery = "", sortBookColumn = 'title', sortBookOrder = '') {
    ...
    fetch(`functions/fetch_missing_pagination.php?page=${page}&itemsPerPage=${itemsPerPage}&search=${encodeURIComponent(searchQuery)}&sortBookColumn=${sortBookColumn}&sortBookOrder=${sortBookOrder}`)
        .then(response => response.json())
        .then(data => {
            totalMissingBooks = data.totalMissingBooks; // Update total books count
            document.getElementById("book-table-container").innerHTML = generateTable(data.missingBookList);
            updatePaginationControls();
        })
        .catch(error => console.error("Error fetching books:", error));
}

function generateTable(missingBookList) {
    const columnNames = ['acc_number', 'class_number', 'title', 'author_name', 'category_name', 'copyright'];
    ...
    const tableHeader = `
        <table id="table">
            ...
            <tbody>
                ${generateTableRows(missingBookList)}
            </tbody>
        </table>
    `;
    return tableHeader;
...
function generateTableRows(missingBookList) {
    if (missingBookList.length === 0) {
        return `
            <tr>
                <td colspan="7">
                    <div class="no-result">
    ...
    return missingBookList.map(book => {
        return `
            <tr>
                ...
                <td>
                    <div class="td-center">
                        <div class="button-view" 
                            data-missing-id="${encodeURIComponent(book.missing_id)}"
...
function updatePaginationControls() {
    ...
    const totalPages = Math.ceil(totalMissingBooks / rowsPerPage);
    ...
    const endEntry = Math.min(currentPage * rowsPerPage, totalMissingBooks);
    entryInfo.textContent = `Showing ${startEntry} to ${endEntry} of ${totalMissingBooks} entries`;
