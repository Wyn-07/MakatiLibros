...
let totalCondemnedBooks = 0;
let sortBookColumn = 'date';
let sortBookOrder = 'desc';
let searchQuery = "";

function sortTable(columnIndex) {
    // Map column index to column name for sorting
    const columnNames = ['date','acc_number', 'class_number', 'title', 'author_name', 'category_name', 'copyright'];
...
function loadBooks(page = 1, itemsPerPage = 5, searchQuery = "", sortBookColumn = 'title', sortBookOrder = '') {
    ...
    fetch(`functions/fetch_condemned_pagination.php?page=${page}&itemsPerPage=${itemsPerPage}&search=${encodeURIComponent(searchQuery)}&sortBookColumn=${sortBookColumn}&sortBookOrder=${sortBookOrder}`)
        .then(response => response.json())
        .then(data => {
            totalCondemnedBooks = data.totalCondemnedBooks; // Update total books count
            document.getElementById("book-table-container").innerHTML = generateTable(data.condemnedBookList);
            updatePaginationControls();
        })
        .catch(error => console.error("Error fetching books:", error));
}

function generateTable(condemnedBookList) {
    const columnNames = ['date', 'acc_number', 'class_number', 'title', 'author_name', 'category_name', 'copyright'];
    ...
    const tableHeader = `
        <table id="table">
            ...
            <tbody>
                ${generateTableRows(condemnedBookList)}
            </tbody>
        </table>
    `;
    return tableHeader;
...
function generateTableRows(condemnedBookList) {
    if (condemnedBookList.length === 0) {
        return `
            <tr>
                <td colspan="8">
                    <div class="no-result">
    ...
    return condemnedBookList.map(book => {
        return `
            <tr>
                <td>${book.date}</td>
                ...
                <td>
                    <div class="td-center">
                        <div class="button-view" 
                            data-condemned-id="${encodeURIComponent(book.condemned_id)}"
...
function updatePaginationControls() {
    ...
    const totalPages = Math.ceil(totalCondemnedBooks / rowsPerPage);
    ...
    const endEntry = Math.min(currentPage * rowsPerPage, totalCondemnedBooks);
    entryInfo.textContent = `Showing ${startEntry} to ${endEntry} of ${totalCondemnedBooks} entries`;
