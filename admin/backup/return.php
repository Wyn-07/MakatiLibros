...
<script>
    ...
    function fetchData(searchQuery) {
        fetch('functions/fetch_returned_table.php?search=' + encodeURIComponent(searchQuery))
            .then(response => {
                if (!response.ok) {
    ...
    function updateTable(returned) {
        ...
        if (returned.length === 0) {
            ...
            filteredRows = [];
            originalRows = []; // Reset originalRows if no return found
        } else {
            returned.forEach(returned => {
                const row = document.createElement('tr');
                row.innerHTML = `
                        <td>${returned.return_date}</td>
                        <td>${returned.acc_number}</td>
                        <td>${returned.class_number}</td>
                        <td>${returned.title}</td>
                        <td>${returned.firstname} ${returned.middlename ? returned.middlename + ' ' : ''}${returned.lastname} ${returned.suffix ? returned.suffix : ''}</td>`;
